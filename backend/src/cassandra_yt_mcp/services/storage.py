...
def to_markdown(result: TranscriptResult, metadata: dict[str, object] | None = None) -> str:
    ...
    if upload_date and isinstance(upload_date, str) and len(upload_date) == 8:
        meta_lines.append(f"**Date**: {upload_date[:4]}-{upload_date[4:6]}-{upload_date[6:8]}")
    ...
    if thumbnail:
        lines.extend([f"![Thumbnail]({thumbnail})", ""])
    ...
    for segment in result.segments:
        if prev is not None:
            ...
            if gap >= PAUSE_THRESHOLD_SECONDS or segment.speaker != prev.speaker:
                lines.append("")
        lines.append(
            f"- [{_format_timestamp(segment.start)}] **{segment.speaker or 'Speaker'}**: {segment.text}"
        )
        prev = segment
...
class StorageService:
    ...
    def persist(
        ...
        *,
        ...
        transcript_md_path.write_text(to_markdown(transcript, metadata=metadata), encoding="utf-8")
